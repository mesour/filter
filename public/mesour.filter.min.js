...
fvar mesour=!mesour?{}:mesour;if(!mesour.filter){throw new Error('Widget '+mesour.filter+' is not created. First create mesour.filter widget.');}mesour.filter.applyDropDown=function(a,b,c){if(c!==''){var d=c.id;var e=c.opened;mesour.cookie(a+'-'+d,e)}};mesour.filter.DropDown=function(z,A,B){var C=this;var D=function(a){if(a===null){return mesour.filter.VALUE_NULL}else if(a===true){return mesour.filter.VALUE_TRUE}else if(a===false){return mesour.filter.VALUE_FALSE}return a};this.fixVariable=function(a){return D(a)};this.translateVariable=function(a){if(a===null){return'<i>'+mesour.filter.translations['empty']+'</i>'}else if(a===true){return'<i>'+mesour.filter.translations['true']+'</i>'}else if(a===false){return'<i>'+mesour.filter.translations['false']+'</i>'}return a};var E=z.attr('data-type');var F,checkers,mouseIn=false;var G=function(){};var H=function(){var a=z.find('.box-inner').find('ul');a.find('li:not(.all-select-li):not(.all-select-searched-li)').remove()};this.destroy=function(){H()};this.create=function(a,b){G(a,b)};var I=function(a){mesour.filter.applyDropDown(B.getName(),B.getDropDownLink(),a)};G=function(d,e){d=!d?B.getData():d;if(!d)return;var f={};for(var x=0;x<d.length;x++){if(typeof d[x][A]==='undefined'){throw new Error('MesourFilterDropDownException: Column "'+A+'" does not exists in data.');}if(!f[d[x][A]]){f[D(d[x][A])]={val:D(d[x][A]),translated:C.translateVariable(d[x][A]),keys:[x]}}else{f[d[x][A]].keys.push(x)}}if(!E){var g=z.find('.box-inner').find('ul');for(var y in f){if(!f[y].val&&Number(f[y].val)!==0)continue;var h=$('<li>'),id=A+((f[y].val&&typeof f[y].val.replace==='function')?f[y].val.replace(' ',''):f[y].val);h.append('<input type="checkbox" class="checker" data-value="'+f[y].val+'" id="'+id+'">');h.append('&nbsp;');h.append('<label for="'+id+'">'+f[y].translated+'</label>');g.append(h)}}else if(E==='date'){var j=[],months={},special={};for(var y in f){if(!f[y].val)continue;var k=isNaN(f[y].val);if(f[y].val){if(f[y].val===mesour.filter.VALUE_NULL||f[y].val===mesour.filter.VALUE_TRUE||f[y].val===mesour.filter.VALUE_FALSE){special[f[y]]=f[y]}}var l=k?mesour.core.strtotime(f[y].val):f[y].val;var m=mesour.core.phpDate('Y',l);var n=mesour.core.phpDate('n',l);var o=mesour.core.phpDate('j',l);if(j.indexOf(m)===-1){j.push(m)}if(!months[m]){months[m]={};months[m]['months']=[];months[m]['days']={}}if(months[m]['months'].indexOf(n)===-1){months[m]['months'].push(n)}if(!months[m]['days'][n]){months[m]['days'][n]=[]}if(months[m]['days'][n].indexOf(o)===-1){months[m]['days'][n].push(o)}}j.sort(function(a,b){return b-a});var g=z.find('.box-inner').find('ul');for(var i in special){if(!special.hasOwnProperty(i)){continue}var h=$('<li>'),id=A+((special[i].val&&typeof special[i].val.replace==='function')?special[i].val.replace(' ',''):special[i].val);h.append('<input type="checkbox" class="checker" data-value="'+special[i].val+'" id="'+id+'">');h.append('&nbsp;');h.append('<label for="'+id+'">'+special[i].translated+'</label>');g.append(h)}for(var a in j){if(!j.hasOwnProperty(a)){continue}var p=$('<li>');p.append('<span class="glyphicon glyphicon-plus toggle-sub-ul"></span>');p.append('&nbsp;');p.append('<input type="checkbox" class="checker">');p.append('&nbsp;');p.append('<label>'+j[a]+'</label>');p.append('<span class="close-all">(<a href="#">Close all</a>)</span>');var q=$('<ul class="toggled-sub-ul">');p.append(q);months[j[a]].months.sort(function(a,b){return a-b});var n=months[j[a]].months;for(var b in n){var r=$('<li>');r.append('<span class="glyphicon glyphicon-plus toggle-sub-ul"></span>');r.append('&nbsp;');r.append('<input type="checkbox" class="checker">');r.append('&nbsp;');r.append('<label>'+mesour.filter.translations.months[n[b]]+'</label>');q.append(r);var s=$('<ul class="toggled-sub-ul">');r.append(s);months[j[a]].days[n[b]].sort(function(a,b){return a-b});var t=months[j[a]].days[n[b]];for(var c in t){var u=mesour.core.strtotime(j[a]+'-'+n[b]+'-'+t[c]);var v=k?mesour.core.phpDate(B.getPhpDateFormat(),u):u;var w=$('<li>');w.append('<span class="glyphicon">&nbsp;</span>');w.append('<input type="checkbox" class="checker" data-value="'+v+'">');w.append('&nbsp;');w.append('<label>'+t[c]+'</label>');s.append(w)}}g.append(p)}}if(e){checkers=new mesour.filter.Checkers(C)}};G();this.getName=function(){return A};this.getType=function(){return!E?'text':E};this.getElement=function(){return z};this.getValues=function(a){var b=B.getValues(A);if(!a){return b}else{if(!b[a]){return{}}else{return b[a]}}};this.setValues=function(a,b){var c=B.getValues(A);c[b]=a;B.setValues(c,A)};this.unsetValues=function(a){var b=B.getValues(A);delete b[a];B.setValues(b,A)};this.getFilter=function(){return B};F=new mesour.filter.CustomFilter(this);checkers=new mesour.filter.Checkers(this);this.update=function(){var a=C.getValues(),toggle_button=z.find('.dropdown-toggle'),menu=z.find('.dropdown-menu'),first_submenu=menu.children('.dropdown-submenu');toggle_button.find('.glyphicon-ok').hide();first_submenu.find('.glyphicon').closest('button').hide();if(a){if(a.custom&&a.custom.operator){toggle_button.find('.glyphicon-ok').show();first_submenu.find('.glyphicon').closest('button').show()}if(a.checkers&&typeof a.checkers[0]!=='undefined'){toggle_button.find('.glyphicon-ok').show();for(var x=0;x<a.checkers.length;x++){checkers.check(a.checkers[x])}}}};this.toggle=function(){if(C.isOpened()){C.close()}else{C.open()}};this.isOpened=function(){return z.hasClass('open')};this.open=function(){B.closeAll(z);z.addClass('open');I({id:C.getName(),opened:1})};this.close=function(){C.update();z.removeClass('open');I({id:C.getName(),opened:0})};z.on({mouseenter:function(){mouseIn=true},mouseleave:function(){mouseIn=false}});$('.mesour-filter-modal').on({mouseenter:function(){mouseIn=true},mouseleave:function(){mouseIn=false}});$('html').on('click.filter-el-'+A,function(){if(C.isOpened()&&!mouseIn){C.close()}});z.children('button').on('click',function(e){e.preventDefault();B.closeAll(z);C.toggle(z)});z.find('.reset-filter').on({click:function(){C.unsetValues('custom');C.update();C.save();B.apply()},mouseenter:function(){$(this).removeClass('btn-success').addClass('btn-danger')},mouseleave:function(){$(this).removeClass('btn-danger').addClass('btn-success')}});z.find('.close-filter').on('click',function(e){e.preventDefault();C.update();C.close()});this.save=function(){var a=checkers.getChecked();if(a.length>0){C.setValues(C.getFilter().generateNextPriority(),'priority');C.setValues(a,'checkers');C.setValues(E!=='date'?'text':'date','type')}else{C.unsetValues('priority');C.unsetValues('checkers')}};this.update()};