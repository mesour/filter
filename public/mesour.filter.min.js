...
var mesour=!mesour?{}:mesour;if(!mesour.filter){throw new Error('Widget '+mesour.filter+' is not created. First create mesour.filter widget.');}mesour.filter.applyDropDown=function(a,b,c){if(c!==''){var d=c.id;var e=c.opened;mesour.cookie(a+'-'+d,e)}};mesour.filter.DropDown=function(w,z,A){var B=this;var C=function(a){if(a===null){return mesour.filter.VALUE_NULL}else if(a===true){return mesour.filter.VALUE_TRUE}else if(a===false){return mesour.filter.VALUE_FALSE}return a};this.fixVariable=function(a){return C(a)};this.translateVariable=function(a){if(a===null){return'<i>'+mesour.filter.translations['empty']+'</i>'}else if(a===true){return'<i>'+mesour.filter.translations['true']+'</i>'}else if(a===false){return'<i>'+mesour.filter.translations['false']+'</i>'}return a};var D=w.attr('data-type');var E,checkers,mouseIn=false;var F=function(){};var G=function(){var a=w.find('.box-inner').find('ul');a.find('li:not(.all-select-li):not(.all-select-searched-li)').remove()};this.destroy=function(){G()};this.create=function(a,b){F(a,b)};var H=function(a){mesour.filter.applyDropDown(A.getName(),A.getDropDownLink(),a)};F=function(d,e){d=!d?A.getData():d;if(!d)return;var f={};for(var x=0;x<d.length;x++){if(typeof d[x][z]==='undefined'){throw new Error('MesourFilterDropDownException: Column "'+z+'" does not exists in data.');}if(!f[d[x][z]]){f[C(d[x][z])]={val:C(d[x][z]),translated:B.translateVariable(d[x][z]),keys:[x]}}else{f[d[x][z]].keys.push(x)}}if(!D){var g=w.find('.box-inner').find('ul');for(var y in f){if(!f[y].val&&Number(f[y].val)!==0)continue;var h=$('<li>'),id=z+((f[y].val&&typeof f[y].val.replace==='function')?f[y].val.replace(' ',''):f[y].val);h.append('<input type="checkbox" class="checker" data-value="'+f[y].val+'" id="'+id+'">');h.append('&nbsp;');h.append('<label for="'+id+'">'+f[y].translated+'</label>');g.append(h)}}else if(D==='date'){var i=[],months={};for(var y in f){if(!f[y].val)continue;var j=isNaN(f[y].val);var k=j?mesour.core.strtotime(f[y].val):f[y].val;var l=mesour.core.phpDate('Y',k);var m=mesour.core.phpDate('n',k);var n=mesour.core.phpDate('j',k);if(i.indexOf(l)===-1){i.push(l)}if(!months[l]){months[l]={};months[l]['months']=[];months[l]['days']={}}if(months[l]['months'].indexOf(m)===-1){months[l]['months'].push(m)}if(!months[l]['days'][m]){months[l]['days'][m]=[]}if(months[l]['days'][m].indexOf(n)===-1){months[l]['days'][m].push(n)}}i.sort(function(a,b){return b-a});var g=w.find('.box-inner').find('ul');for(var a in i){var o=$('<li>');o.append('<span class="glyphicon glyphicon-plus toggle-sub-ul"></span>');o.append('&nbsp;');o.append('<input type="checkbox" class="checker">');o.append('&nbsp;');o.append('<label>'+i[a]+'</label>');o.append('<span class="close-all">(<a href="#">Close all</a>)</span>');var p=$('<ul class="toggled-sub-ul">');o.append(p);months[i[a]].months.sort(function(a,b){return a-b});var m=months[i[a]].months;for(var b in m){var q=$('<li>');q.append('<span class="glyphicon glyphicon-plus toggle-sub-ul"></span>');q.append('&nbsp;');q.append('<input type="checkbox" class="checker">');q.append('&nbsp;');q.append('<label>'+mesour.filter.translations.months[m[b]]+'</label>');p.append(q);var r=$('<ul class="toggled-sub-ul">');q.append(r);months[i[a]].days[m[b]].sort(function(a,b){return a-b});var s=months[i[a]].days[m[b]];for(var c in s){var t=mesour.core.strtotime(i[a]+'-'+m[b]+'-'+s[c]);var u=j?mesour.core.phpDate(A.getPhpDateFormat(),t):t;var v=$('<li>');v.append('<span class="glyphicon">&nbsp;</span>');v.append('<input type="checkbox" class="checker" data-value="'+u+'">');v.append('&nbsp;');v.append('<label>'+s[c]+'</label>');r.append(v)}}g.append(o)}}if(e){checkers=new mesour.filter.Checkers(B)}};F();this.getName=function(){return z};this.getType=function(){return!D?'text':D};this.getElement=function(){return w};this.getValues=function(a){var b=A.getValues(z);if(!a){return b}else{if(!b[a]){return{}}else{return b[a]}}};this.setValues=function(a,b){var c=A.getValues(z);c[b]=a;A.setValues(c,z)};this.unsetValues=function(a){var b=A.getValues(z);delete b[a];A.setValues(b,z)};this.getFilter=function(){return A};E=new mesour.filter.CustomFilter(this);checkers=new mesour.filter.Checkers(this);this.update=function(){var a=B.getValues(),toggle_button=w.find('.dropdown-toggle'),menu=w.find('.dropdown-menu'),first_submenu=menu.children('.dropdown-submenu');toggle_button.find('.glyphicon-ok').hide();first_submenu.find('.glyphicon').closest('button').hide();if(a){if(a.custom&&a.custom.operator){toggle_button.find('.glyphicon-ok').show();first_submenu.find('.glyphicon').closest('button').show()}if(a.checkers&&typeof a.checkers[0]!=='undefined'){toggle_button.find('.glyphicon-ok').show();for(var x=0;x<a.checkers.length;x++){checkers.check(a.checkers[x])}}}};this.toggle=function(){if(B.isOpened()){B.close()}else{B.open()}};this.isOpened=function(){return w.hasClass('open')};this.open=function(){A.closeAll(w);w.addClass('open');H({id:B.getName(),opened:1})};this.close=function(){B.update();w.removeClass('open');H({id:B.getName(),opened:0})};w.on({mouseenter:function(){mouseIn=true},mouseleave:function(){mouseIn=false}});$('.mesour-filter-modal').on({mouseenter:function(){mouseIn=true},mouseleave:function(){mouseIn=false}});$('html').on('click.filter-el-'+z,function(){if(B.isOpened()&&!mouseIn){B.close()}});w.children('button').on('click',function(e){e.preventDefault();A.closeAll(w);B.toggle(w)});w.find('.reset-filter').on({click:function(){B.unsetValues('custom');B.update();B.save();A.apply()},mouseenter:function(){$(this).removeClass('btn-success').addClass('btn-danger')},mouseleave:function(){$(this).removeClass('btn-danger').addClass('btn-success')}});w.find('.close-filter').on('click',function(e){e.preventDefault();B.update();B.close()});this.save=function(){var a=checkers.getChecked();if(a.length>0){B.setValues(B.getFilter().generateNextPriority(),'priority');B.setValues(a,'checkers');B.setValues(D!=='date'?'text':'date','type')}else{B.unsetValues('priority');B.unsetValues('checkers')}};this.update()};